...
dataset = dataset.map(lambda sample: {
	"input_ids": tokenizer.apply_chat_template(sample,
	                                           ...
	                                           return_tensors="pt")
}, input_columns="query", remove_columns="query",  num_proc=16)

dataset = dataset.map(lambda examples: {
    'input_ids': [x[0] for x in examples['input_ids']], 'label': examples['label']}
                                , batched=True)
...
optimizer = PagedLion32bit(filter(lambda p: p.requires_grad, base_model.parameters()), lr=rloo_config.learning_rate)
...
tuner = RLOOTrainer(
	config=rloo_config,
	policy=base_model_with_adapter,
 	ref_policy=base_model_with_adapter,
	reward_model=SA_model,
	tokenizer=tokenizer,
